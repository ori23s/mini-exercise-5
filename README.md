...
## Part 7:
This code takes part 6 and synchronizes the incrementation, this way we can get all the way to 100 million. The code indeed succeeds to increment the value all the way to 100 million throught the intrinsic lock where all threads read the last value and only one increments at a time. The code also tracks the amount of time the program takes and we get 893 on first run and higher numbers on later runs giving us runtimes of about 950 milliseconds. This expresses the extreme performance difference the synchronization causes. Without synchronization takes 22 milliseconds compared to the 950. The synchronization hurts our performance a lot.
